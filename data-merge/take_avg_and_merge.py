...
df_client_avg = df_client.groupby("ap_name", as_index=False)[
    "client_count"
].mean()
df_client_avg.columns = ["ap_name", "avg_client_count"]

df_cpu_avg = df_cpu.groupby("ap_name", as_index=False)[
    "cpu_usage_ratio"
].mean()
...
df_5g_avg = df_5g.groupby("ap_name", as_index=False)[
    "signal_dbm"
].mean()
df_5g_avg.columns = ["ap_name", "avg_signal_5g_dbm"]

df_24g_avg = df_24g.groupby("ap_name", as_index=False)[
    "signal_dbm"
].mean()
df_24g_avg.columns = ["ap_name", "avg_signal_24g_dbm"]

merged_df_p1 = pd.merge(
    pd.DataFrame(df_client_avg), pd.DataFrame(df_cpu_avg), on="ap_name", how="inner"
)
merged_df_p2 = pd.merge(
    merged_df_p1, pd.DataFrame(df_memory_avg), on="ap_name", how="inner"
)

merged_df_signal = pd.merge(
    pd.DataFrame(df_5g_avg), pd.DataFrame(df_24g_avg), on="ap_name", how="outer"
)
merged_df_signal['avg_signal_dbm'] = merged_df_signal[['avg_signal_5g_dbm', 'avg_signal_24g_dbm']].mean(axis=1)

merged_df_final = pd.merge(
    merged_df_p2, merged_df_signal[['ap_name', 'avg_signal_dbm']], on="ap_name", how="inner"
)
