...
merged_df_p1 = pd.merge(
    pd.DataFrame(df_client_avg), pd.DataFrame(df_cpu_avg), on="ap_name", how="outer"
)
merged_df_p2 = pd.merge(
    merged_df_p1, pd.DataFrame(df_memory_avg), on="ap_name", how="outer"
)
merged_df_p3 = pd.merge(
    merged_df_p2, pd.DataFrame(df_5g_avg), on="ap_name", how="outer"
)
merged_df_final = pd.merge(
    merged_df_p3, pd.DataFrame(df_24g_avg), on="ap_name", how="outer"
)
